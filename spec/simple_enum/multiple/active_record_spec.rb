...
RSpec.describe SimpleEnum::Multiple::Accessors, active_record: true do
  shared_examples_for 'multiple accessor' do |accessor, source|
    ...
    context 'generate_enum_dirty_methods_for' do
      ...
      it 'does not respond to #favorites_changed?' do
        expect(subject).to_not respond_to(
          :favorites_changed?
        )
      end
      ...
      context 'with: :dirty' do
        ...
        it 'delegates #favorites_changed? to accessor' do
          expect(subject.favorites_changed?).to(
            be_truthy
          )
        end
    ...
    context 'generate_enum_attribute_methods_for' do
      ...
      context 'with: :attribute' do
        ...
        it 'delegates #iphone? to accessor' do
          subject.favorites = [:ipad]
          expect(subject.iphone?).to(
            be_falsey
          )
        end

        it 'delegates #ipad! to accessor' do
          expect(subject.ipad!).to eq [
            'ipad'
          ]
        end
      ...
      context 'with a prefix' do
        ...
        it 'delegates #favorites_ipad? to accessor' do
          subject.favorites = [:ipad]
          expect(subject.favorites_ipad?).to(
            be_truthy
          )
        end
        ...
        it 'delegates #favorites_iphone? to accessor' do
          subject.favorites = [:ipad]
          expect(subject.favorites_iphone?).to(
            be_falsey
          )
        end

        it 'delegates #favorites_ipad! to accessor' do
          expect(subject.favorites_ipad!).to eq [
            'ipad'
          ]
        end
        ...
        it 'delegates #favorites_iphone! to accessor' do
          subject.favorites = [:ipad]
          expect(subject.favorites_iphone!).to eq [
            'iphone'
          ]
        end
      end
    ...
    context 'generate_enum_scope_methods_for', active_record: true do
      ...
      context 'with: :attribute' do
        ...
        context '.iphones' do
          ...
          it_behaves_like(
            'delegates to accessor#scope', :iphone
          )
        end
        ...
        context '.ipads' do
          subject { klass.ipads }
          it_behaves_like(
            'delegates to accessor#scope', :ipad
          )
        end
  ...
  context 'accessor: :bitwise' do
    shared_examples_for 'delegates to accessor#scope' do |value|
      it 'delegates to #scope' do
        subject.count == klass.all.select do |r|
          r.favorites.include?(value)
        end.size
      end
    end

    it_behaves_like(
      'multiple accessor', :bitwise,
      :bitwise_cds
    )
  end
end
