# Frozen-String-Literal: true
# Encoding: utf-8

require 'simple_enum/multiple/proxy'

module SimpleEnum
  module Multiple
    class Accessors < SimpleEnum::Accessors::Accessor
      def initialize(name, enum, source = nil, prefix = nil)
        super

        @source = source.to_s.presence
        s = SimpleEnum.suffix.pluralize
        n = @name.singularize
        @source ||= "#{n}#{s}"
      end

      def read(object)
        Proxy.new(
          read_before_type_cast(object), self
        )
      end

      def write(object, keys)
        keys = filter_keys(Array.wrap(keys))
        write_after_type_cast(
          object, fetch_values(keys)
        ) && keys
      end

      def selected?(object, key = nil)
        current = read_before_type_cast(object)
        current.any? &&
          if key
            current.all? do
              current.include?(
                enum.value(
                  key
                )
              )
            end
          else
            true
          end
      end

      def was(object)
        fetch_keys(
          object.send(
            :attribute_was, source
          ).to_a
        )
      end

      def scope(_collection, _value)
        raise NoMethodError, 'scopes for accessor: :multiple are not supported!'
      end

      def filter_keys(keys)
        keys.select do |key|
          enum.value(key)
        end
      end

      def fetch_keys(values)
        values.map do |value|
          enum.key(value)
        end
      end

      def fetch_values(keys)
        keys.map do |key|
          enum.value(key)
        end
      end

      private

      def read_before_type_cast(object)
        result = super
        return result if result

        result = []
        write_after_type_cast(
          object, result
        )

        result
      end
    end
  end
end
